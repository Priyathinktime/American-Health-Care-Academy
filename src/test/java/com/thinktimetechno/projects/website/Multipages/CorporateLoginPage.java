...
public class CorporateLoginPage {
	...
	public void course_selection() {
		...
		for (int i = 1; i <= numberOfRows; i++) {
			...
			// Click on the Course dropdown (Select2 trigger) (//div[contains(@id,
			// 's2id_codeauth_pre_paid')])
			
			WebUI.clickElementWithJs(By.xpath("(//div[@class='select2-result-label'])[2]"));
			//div[@id='select2-drop']/child::ul/li[2]
		//	((//div[@id='s2id_codeauth_pre_paid'])[" + i + "]//span)[2]
//			// Wait for options to be visible
//			WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(10));
//			wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='select2-results']//li")));
//			WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(10));
//			wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='select2-drop']/child::ul/li[2]")));
//
////			// Click the first course option from dropdown
//			WebUI.clickElementWithJs(By.xpath("//div[@id='select2-drop']/child::ul/li[2]"));
		
		}
	}
	public void course_selection_01() {
	...
	public void click_nxt() {
		...
		WebUI.clickElement(By.xpath("//button[contains(text(),'Next')]"));

	}

	public void click_nxt1() {

		WebUI.clickElement(By.xpath("//button[text()='Next']"));
	...
	public void click_InviteNewEmployee() {
		...
		WebUI.clickElement(By.xpath("(//a[contains(text(),'Invite New Employee')])[2]"));
	}
	...
	public void groupUserEnterQuantityForCourse(int quantity, String courseName) {
		...
		for (WebElement courseBlock : courseBlocks) {
			// Check if the course block is visible
			if (!courseBlock.isDisplayed()) {
				continue; // Skip to next one if not visible
			}
